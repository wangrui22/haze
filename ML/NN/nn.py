...
def test_regression(plots=False):
    ...
    #functions.
    param=((1,0,0),(10, logistic, logistic_prime),(5, logistic, logistic_prime),(1,identity, identity_prime))
    #Set learning rate.
    rates=[0.05]
    predictions=[]
    for rate in rates:
        N=NeuralNetwork(X,y,param)
        N.train(200, learning_rate=rate)
    ...
    if plots:
        ...
        for data in predictions:
            ax.plot(X,data[1],label="Learning Rate: "+str(data[0]) , linewidth =2 , color='red')
        ax.legend()
    plt.show()
